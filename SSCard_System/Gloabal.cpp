...
int DataCenter::PrintCard(SSCardBaseInfoPtr& pSSCardInfo, QString strPhoto, QString& strMessage, bool bPrintText)
{
	...
	string strDatagram = 0;
	do
	{
		if (pSScardSerivce)
		{
			pSScardSerivce->GetStageStatus(strStage, nResult, strDatagram);
			if (QSucceed(nResult))
				break;
		}
		...
		if (pSScardSerivce)
		{
			pSScardSerivce->UpdateStage(strStage, 0, "");
		}
...
int	 DataCenter::WriteCard(QString& strMessage)
{
	int nResult = -1;
	string strJsonIn, strJsonOut;
	QString strInfo;
